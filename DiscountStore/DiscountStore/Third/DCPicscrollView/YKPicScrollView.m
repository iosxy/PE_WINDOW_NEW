...
#import "YKPicScrollView.h"
#import "YKWebImageManager.h"

@interface YKPicScrollView () <UIScrollViewDelegate>

@property (nonatomic,copy) NSArray *imageData;

@end

@implementation YKPicScrollView{
...
+ (instancetype)picScrollViewWithFrame:(CGRect)frame WithImageUrls:(NSArray<NSString *> *)imageUrl {
    return  [[YKPicScrollView alloc] initWithFrame:frame WithImageNames:imageUrl];
}
...
- (UIImage *)setImageWithIndex:(NSInteger)index {
    ...
    //从内存缓存中取,如果没有使用占位图片
    UIImage *image = [[[YKWebImageManager shareManager] webImageCache] valueForKey:_imageData[index]];
...
- (void)setImageData:(NSArray *)ImageNames {
    ...
    if (_isNetwork) {
        ...
        for (NSString *name in ImageNames) {
            [temp addObject:[UIImage imageNamed:@""]];
        }
...
- (void)getImage {
    ...
    for (NSString *urlSting in _imageData) {
        [[YKWebImageManager shareManager] downloadImageWithUrlString:urlSting];
    }
